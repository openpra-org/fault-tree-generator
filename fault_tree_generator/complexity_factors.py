...
class ComplexityFactorError(Exception):
...
class ComplexityFactors:
    ...
    def set_min_max_prob(self, min_value, max_value):
        ...
        if min_value < 0 or min_value > 1:
            raise ComplexityFactorError("Min probability must be in [0, 1] range.")
        if max_value < 0 or max_value > 1:
            raise ComplexityFactorError("Max probability must be in [0, 1] range.")
        if min_value > max_value:
            raise ComplexityFactorError("Min probability > Max probability.")
    ...
    def set_common_event_factors(self, common_b, common_g, parents_b,
        ...
        if parents_b < 2 or parents_b > max_parent:
            raise ComplexityFactorError("parents_b not in [2, " + str(max_parent) + "].")
        if parents_g < 2 or parents_g > max_parent:
            raise ComplexityFactorError("parents_g not in [2, " + str(max_parent) + "].")
    ...
    def set_num_factors(self, num_args, num_basic, num_house=0, num_ccf=0):
        ...
        """
        if num_args < 2:
            raise ComplexityFactorError("avg. # of gate arguments can't be less than 2.")
        if num_basic < 1:
            raise ComplexityFactorError("# of basic events must be more than 0.")
        if num_house < 0:
            raise ComplexityFactorError("# of house events can't be negative.")
        if num_ccf < 0:
            raise ComplexityFactorError("# of CCF groups can't be negative.")
        if num_house >= num_basic:
            raise ComplexityFactorError("Too many house events.")
        if num_ccf > num_basic / num_args:
            raise ComplexityFactorError("Too many CCF groups.")
    ...
    def calculate(self):
        ...
        """
        self.__max_args = ComplexityFactors.__calculate_max_args(self.num_args,
                                                                 self.__norm_weights)
    ...
    def set_gate_weights(self, weights):
        ...
        """
        if not weights:
            raise ComplexityFactorError("No weights are provided")
        if [i for i in weights if i < 0]:
            raise ComplexityFactorError("Weights cannot be negative")
        if len(weights) > len(ComplexityFactors.__OPERATORS):
            raise ComplexityFactorError("Too many weights are provided")
        if sum(weights) == 0:
            raise ComplexityFactorError("At least one non-zero weight is needed")
        if len(weights) > 3 and not sum(weights[:3]):
            raise ComplexityFactorError("Cannot work with only XOR or NOT gates")

        self.__weights_g = weights[:]
        for _ in range(len(ComplexityFactors.__OPERATORS) - len(weights)):
    ...
    def get_random_operator(self):
        ...
        while self.__cum_dist[bin_num] <= r_num:
            bin_num += 1
        return ComplexityFactors.__OPERATORS[bin_num - 1]
    ...
    def constrain_num_gate(self, num_gate):
        ...
        """
        if num_gate < 1:
            raise ComplexityFactorError("# of gates can't be less than 1.")
        if num_gate * self.num_args <= self.num_basic:
            raise ComplexityFactorError("Not enough gates and avg. # of args to achieve the # of basic events")
